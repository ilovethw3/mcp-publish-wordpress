/**
 * MCP Proxy: Approve Article Only
 * 
 * 代理Web UI前端对MCP服务器approve_article_only工具的调用
 * 使用服务端API密钥进行认证
 */
...
interface ApproveArticleOnlyRequest {
  article_id: number;
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // 解析请求体
    const body: ApproveArticleOnlyRequest = await request.json();
    ...
    console.log(`[WEB-UI] 🔐 代理调用 MCP approve_article_only，文章ID: ${article_id}`);
    console.log(`[WEB-UI] 📝 审批备注: ${reviewer_notes || '(无)'}`);
    
    // 使用 MCP 客户端调用 approve_article_only 工具
    const mcpClient = getMCPClient({ apiKey });
    const result = await mcpClient.approveArticleOnly(article_id, reviewer_notes);
